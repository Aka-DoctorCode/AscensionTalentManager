...
local function EnsureDB()
    if type(AscensionTalentDB) ~= "table" then
        AscensionTalentDB = {}
    end
    for k, v in pairs(DEFAULTS) do
        if AscensionTalentDB[k] == nil then
            AscensionTalentDB[k] = v
        end
    end
end
...
local function Log(msg, ...)
    if AscensionTalentDB and AscensionTalentDB.debug then
...
local function CheckAndPromptSwitch(force)
    ...
    if not AscensionTalentDB or not AscensionTalentDB.enabled then return end
    ...
    if not AscensionTalentDB.perSpec then
        AscensionTalentDB.perSpec = {}
    end

    if not AscensionTalentDB.perSpec[specID] then
        AscensionTalentDB.perSpec[specID] = {}
    end
    -- AscensionTalentDB.perSpec[specID] = AscensionTalentDB.perSpec[specID] or {}

    local desiredLoadoutName = AscensionTalentDB.perSpec[specID][context]
...
SlashCmdList["ASCENSIONTALENTS"] = function(msg)
    local cmd = msg:lower()
    if cmd == "debug" then
        if AscensionTalentDB then
            AscensionTalentDB.debug = not AscensionTalentDB.debug
            print("Ascension Debug:", AscensionTalentDB.debug)
        end
