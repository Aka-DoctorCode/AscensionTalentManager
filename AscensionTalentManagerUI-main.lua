...
local function CreateSafeDropdown(parent, ctxKey, width)
    ...
    frame.UpdateSelection = function(self)
        local specID = GetSpecID()
        local val = "-"
        if specID and AscensionTalentDB and AscensionTalentDB.perSpec[specID] then
            val = AscensionTalentDB.perSpec[specID][ctxKey] or "-"
        end
            ...
            for i, name in ipairs(options) do
                ...
                if not btn then
                    ...
                    btn:SetScript("OnClick", function(b)
                        ...
                        local specID = GetSpecID()
                        if specID and AscensionTalentDB then
                            if not AscensionTalentDB.perSpec[specID] then AscensionTalentDB.perSpec[specID] = {} end
                            AscensionTalentDB.perSpec[specID][ctxKey] = (selected ~= "-") and selected or nil
                        end
...
local function CreateConfigFrame()
    ...
    title:SetText("Ascension Talents")
...
local function CreatePromptFrame()
    ...
    PromptFrame.ApplySwitch = function(self)
        ...
        if result then
            print("|cff00ff00[Ascension]|r Switching to " .. (self.targetName or "..."))
            local specID = GetSpecID()
